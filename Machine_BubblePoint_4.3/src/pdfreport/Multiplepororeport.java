...
public class Multiplepororeport {
	...
	void resulttable(List<DatareadN> d) {
		...
		for (int j = 0; j < d.size(); j++) {
			...
			if (j % 2 == 0) {
				...
				PdfPCell record4 = new PdfPCell(new Paragraph(bpp+ " psi",
						tabledata));
				record4.setBorder(0);
				// record4.setBorder(record4.LEFT);
				record4.setBorderColor(getColor(6));
				...
				PdfPCell record5 = new PdfPCell(new Paragraph(bpd+ " micron",
						tabledata));
				record5.setBorder(1);
				record5.setBorder(record5.RIGHT);
				record5.setBorderColor(new BaseColor(130, 130, 130));
	...
	void resultgraph(File f) {
		...
		PdfPCell cell2 = new PdfPCell(new Paragraph("Differential Time", rowhed));
		...
		PdfPCell cell5 = new PdfPCell(new Paragraph("Differential Pressure", rowhed));
		...
		PdfPCell ucell3 = new PdfPCell(new Paragraph("(psi)", unitlabrow));
	...
	void rowData(List<DatareadN> d) {
		...
		for (int k = 0; k < d.size(); k++) {
			...
			dt = dr1.getValuesOf("" + dr1.data.get("Dt"));
				...
				if (j % 45 == 0 && j != 0) {
					...
					PdfPCell r11 = new PdfPCell(
							new Paragraph(Myapp.getRound(
									Double.parseDouble("" + flow.get(j)), 2),
									tabledata));
					r11.setBorder(1);
					...
					r55.setBorder(1);
					r55.setBorder(r55.BOTTOM);
					r55.setBorderColor(new BaseColor(130, 130, 130));
					// r55.setBackgroundColor(backcellcol);
					r55.setPaddingTop(0);
					r55.setFixedHeight(14f);
